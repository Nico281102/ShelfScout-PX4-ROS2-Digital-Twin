...
**Low-light policy:** `LOW_LIGHT` is treated purely as an ambient inspection event to toggle the torch via `torch_controller`; it does not trigger mission fallbacks or state changes inside the FSM.
...
> **Notes:**  
> - Fallback triggers (`battery_warning`, `battery_critical`) are evaluated **globally every tick** and may pre-empt *any* state. The arrows below show the common sources, but pre-emption is not limited to them.  
> - Link loss is not modelled inside the FSM; PX4’s native failsafe owns the behaviour once Offboard setpoints stop.  
> - `LOW_LIGHT` is ignored by the FSM; it only drives the torch bridge.  
...
- **Fallback not firing**: ensure triggers in the mission YAML match the supported events (`battery_warning`, `battery_critical`); link loss is handled directly by PX4 when Offboard heartbeats stop.
...
- LOW_LIGHT detection for the torch was unreliable because ambient sun dominated.
...
- LOW_LIGHT events behave deterministically without ambient drift, keeping torch toggling stable.
